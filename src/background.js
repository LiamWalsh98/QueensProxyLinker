...
var contextMenuItem = {
    "id": "openWithProxy",
    "title": "Open link with Queen's Proxy",
    "contexts": ["link"]
};
...
chrome.contextMenus.onClicked.addListener(clicked => {
    if (clicked.menuItemId == "openWithProxy") {
        var proxyUrl = checkField(clicked.linkUrl)
        if (proxyUrl === null) {
            alert("Could not open URL with Queen's Proxy");
        }
        else {
            chrome.tabs.update({
                url: "https://login.proxy.queensu.ca/login?url=" + proxyUrl
            });
        }
    }
});
